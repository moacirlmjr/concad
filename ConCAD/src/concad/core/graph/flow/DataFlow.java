package concad.core.graph.flow;
...
/**
 * This class is responsible to hold the complete flow of a vulnerability
 * starting where the vulnerability was introduced until where it was exploited.
 * 
...
public class DataFlow {
	...
	/**
	 * The parent object because we need to navigate from the parent to its
	 * children and also on the opposite direction.
	 */
	...
	/**
	 * The message that will be displayed to the user informing that this object
	 * is vulnerable.
	 */
	private String message;
	/**
	 * All the possible flows that the vulnerability could reach, but some of
	 * them might end up being not vulnerable.
	 */
	...
	public DataFlow(Expression root) {
		this.root = root;
		children = new ArrayList<DataFlow>();
		allVulnerablePaths = new ArrayList<List<DataFlow>>();
	}
	...
	public DataFlow addNodeToPath(Expression node) {
		...
		// If there is nothing to add or the new value is the same as the
		// present value.
	...
	/**
	 * This method informs to the parent object that a vulnerable path was
	 * found.
	 */
	private void hasVulnerablePath(List<DataFlow> childrenList) {
		List<DataFlow> currentList = new ArrayList<DataFlow>();
	...
	public void replace(DataFlow dataFlow) {
		...
		if (!dataFlow.allVulnerablePaths.isEmpty()) {
			...
			List<List<DataFlow>> copyVulnerablePaths = new ArrayList<List<DataFlow>>(dataFlow.allVulnerablePaths);
			...
			for (Iterator<List<DataFlow>> iterator = copyVulnerablePaths.iterator(); iterator.hasNext();) {
				List<DataFlow> copyList = new ArrayList<DataFlow>(iterator.next());
