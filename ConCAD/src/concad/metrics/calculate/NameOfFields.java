...
public class NameOfFields extends ASTVisitor implements IAttribute {
	...
	public void calculate(ClassMetrics node) {
		...
		TypeDeclaration classDeclaration = (TypeDeclaration) node
				.getDeclaration();
		for (FieldDeclaration field : classDeclaration.getFields()) {
			...
			for (VariableDeclarationFragment declaration : declarations) {
				classFieldsNames.add(declaration.getName()
		...
		for (MethodMetrics methodMetrics : methodsMetrics) {
			List<String> methodFieldsNames1 = (List<String>) methodMetrics
					.getAttribute(MetricNames.MFA);
	...
	public void calculate(MethodMetrics node) {
		...
		if (node.getDeclaration().getBody() != null) {
			...
			node.setAttribute(MetricNames.FON, methodFieldsNames);// method
																	// local
																	// variables
		...
		node.setAttribute(MetricNames.MFA, methodAccessFieldsNames);// access to
																	// class
																	// fields
		node.setAttribute(MetricNames.NOAV, methodVariableAccess);// total
																	// access
																	// name to
																	// variables
	...
	public boolean visit(VariableDeclarationStatement node) {
		...
		if (declarations != null) {
			for (VariableDeclarationFragment declaration : declarations) {
				methodFieldsNames.add(declaration.getName()
						.getFullyQualifiedName());
			}
		}
		return true;
